...
[mi-faser](https://bromberglab.org/project/mifaser/) maps reads to functional databases at the protein level to produce enzyme function profiles. Enabled with `--run_mifaser`.
...
The search phase also writes `*.emapper.hits`, the full hit table for every query. It is a big file and not used very often, so it doesn't get published by default.
...
Because the pipeline runs no read QC, the report currently carries the run summary and the software versions rather than per-sample QC modules. The pipeline has special steps which also allow the software versions to be reported in the MultiQC output for future traceability. For more information about how to use MultiQC reports, see <http://multiqc.info>.
